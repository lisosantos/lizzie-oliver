...
import { Calendar, PenTool, BookHeart } from 'lucide-react';
...
export default function ArticleListPage({ category, onNavigate }: ArticleListPageProps) {
  const [articles, setArticles] = useState<Article[]>([]);
  const [loading, setLoading] = useState(true);

  const pageConfig = {
    'minhas-palavras': {
      title: 'Em Minhas Palavras',
      description: 'Reflexões e escritos pessoais de Lizzie Oliver.',
      icon: BookHeart,
    },
    'sobre-escrita': {
      title: 'Sobre Escrita',
      description: 'Artigos e inspirações sobre a arte de escrever.',
      icon: PenTool,
    },
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <div className="flex justify-center mb-4">
            <Icon className="text-[#C9A227]" size={48} />
          </div>
          <h1
          ...
          <p className="text-lg text-gray-700 mb-6">{config.description}</p>
        ...
        {loading ? (
          ...
          <div className="text-center bg-white rounded-lg p-12 shadow-lg">
            <p className="text-[#4B1E2F] text-lg">Nenhum artigo publicado ainda.</p>
          </div>
        )}
      </div>
    </div>
